...
def regularized_linear_regression(X, y, lambd):
  ...
  #####################################################
  D = X.shape[1]
  XX_regularize = np.dot(X.T, X) + lambd * np.eye(D)
  inv_XX = np.linalg.inv(XX_regularize)
  XY = np.dot(X.T, y)
  w = np.dot(inv_XX, XY)
  #####################################################    
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  """
    ...
    """
  #####################################################
  num_val_samples = Xval.shape[0]
  s_err = np.zeros(shape=(len(lambds),1))
  for k in range(len(lambds)):
    w = regularized_linear_regression(Xtrain, ytrain, lambds[k])
    for i in range(num_val_samples):
      s_err[k] += np.square(np.dot(w.T, Xval[i]) - yval[i]) 
    s_err[k] = s_err[k] / num_val_samples
  m_s_e1 = s_err.tolist() 
  bestlambda = lambds[m_s_e1.index(min(m_s_e1))]
  #####################################################    
  return bestlambda
...
def test_error(w, X, y):
  ...
  for i in range(num_samples):
    qa1 += np.square(np.dot(X[i].T, w) - y[i])
  err = qa1 / num_samples
  return err
