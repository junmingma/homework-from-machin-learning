...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    W = np.concatenate((np.array([b]),w),axis=0)
    X = np.append(np.ones((N,1)),X,axis=1)

    for i in range(0,max_iterations):
        partial_w = np.matmul(sigmoid(np.matmul(X,W.T)) - y,X)
        if (partial_w == 0.0).all():
            break
        else:
            W -= step_size * partial_w / N
...
def multinomial_train(X, y, C, 
    ...
    TODO: add your code here
    """
    W = np.append(np.reshape(b,(-1,1)),w,axis=1)
    X = np.append(np.ones((N,1)),X,axis=1)
    ...
    for k in range(0, max_iterations):
        softmax_val = softmax(np.matmul(X,W.T)) - onehot
        partial_w = np.matmul(softmax_val.T,X)
        if (partial_w == 0.0).all():
            break
        else:
            W -= (step_size * (partial_w / N))
...
def softmax(x):
    return np.exp(x - np.amax(x,axis=1)[:,None]) / np.sum(np.exp(x - np.amax(x,axis=1)[:,None]),axis = 1)[:,None]
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    """
    for c in range(0, C):
    	yprime = np.where(y == c,1,0)
    	temp1 = np.zeros((D,1))
    	temp1, b[c] = binary_train(X, yprime, w0, b0, step_size, max_iterations)
    	w[c] =  temp1.T
