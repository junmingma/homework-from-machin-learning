...
class KMeans():
    ...
    def fit(self, x):
        ...
        for i in range(self.max_iter):
            iteration += 1
            for j in range(k):
                distances[:,j] = np.sum(np.square( np.subtract(miu[j], x)), axis = 1)
            r = np.eye(k)[np.argmin(distances, axis=1)]
            J_new = np.sum(r * distances) / N
            if abs(J - J_new) <= self.e:
                break
            J = J_new
            for m in range(k):
                if np.sum(r[:, m] != 0):
                    miu[m] = np.dot(r[:, m], x) / np.sum(r[:, m])


        R = np.argmax(r, axis=1)
        result = (miu, R, iteration)
        return result
...
class KMeansClassifier():
    ...
    def fit(self, x, y):
        ...
        centroids, R, count = KMeans(self.n_cluster, self.max_iter, self.e).fit(x)
        centroid_labels = np.zeros((self.n_cluster,))
        for i in range(self.n_cluster):
            y_i = y[R == i].astype(int)
    ...
    def predict(self, x):
        ...
        for j in range(self.n_cluster):
            dist[:,j] = np.sum(np.square(x-self.centroids[j]), axis = 1)
        labels = self.centroid_labels[np.argmin(dist, axis=1)]
        # DONOT CHANGE CODE BELOW THIS LINE
        return labels
