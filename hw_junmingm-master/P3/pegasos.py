...
def objective_function(X, y, w, lamb):
    ...
    N = X.shape[0]
    obj_value = 0.5 * lamb * np.square(LA.norm(w)) + np.sum(np.maximum(1-y.T*np.dot(w.T, X.T) , 0)) / float(N) 
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # you need to fill in your solution here
        yita = 1 / float(lamb * iter)
        yx = np.zeros((D, 1))
        for i in A_t:
            if ytrain[i]*np.dot(Xtrain[i], w) < 1:
                yx += (ytrain[i]*Xtrain[i]).reshape(-1, 1)
        w_temp = (1 - yita * lamb) * w + yita / k * yx
        w = np.minimum(1 / float(np.sqrt(lamb) * LA.norm(w_temp)), 1) * w_temp
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    test_acc = np.sum(ytest.T*np.dot(w_l.T, Xtest.T) > 0) / float(Xtest.shape[0])
