...
def forward(pi, A, B, O):
  ...
  alpha[:,0] = pi * B[:,O[0]]
  for i in range(1, N):
    alpha[:,i] = B[:,O[i]] * np.matmul(alpha[:,i-1], A)
  ###################################################
...
def backward(pi, A, B, O):
  ...
  for i in range(N-2, -1, -1):
    beta[:,i] = np.matmul((beta[:,i+1] * B[:,O[i+1]]), A.T)
  ###################################################
...
def viterbi(pi, A, B, O):
  ...
  N = len(O)
  delta = np.zeros((S, N))
  Delta = np.zeros((S, N)).astype(int)
  delta[:,0] = pi * B[:,O[0]]
  Delta[:,0]=np.arange(S)
  for i in range(1, N):
    for j in range(0,S):
      Delta[j,i] = np.argmax(A[:,j] * delta[:,i-1])
      delta[j,i] = B[j,O[i]] * A[Delta[j,i],j] * delta[Delta[j,i],i-1]
  temp = np.argmax(delta[:,N-1])
